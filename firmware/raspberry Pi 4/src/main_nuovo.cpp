...
int main() {
  ...
  globalControllerStatus = jsonConfig["globalControllerStatus"];
  ...
  ControlSystemPITCH pitchControl = ControlSystemPITCH(minForceRoll, maxForceRoll, minErrorImu*DEGtoRAD, weight, buoyancy,
  ...
  ControlSystemROLL rollControl = ControlSystemROLL(minForcePitch, maxForcePitch, minErrorImu*DEGtoRAD, weight, buoyancy, 
  ...
  while (true) {
    ...
    if(milliss-last_controller_time > INTERVAL_CONTROLLORE){
        ...
        readSensorsData();
        if (arm) {
          if (globalControllerStatus) controlSystemCallFunction(zControl, pitchControl, rollControl);
...
void loopMotori(msg_pt msg) {
  ...
  //changeControllerStatus(depth, Z_URemap); //ATTENZIONEEE
...
void readSensorsData(){
  ...
  if (referencePitch_new != 0)
    pitch = referencePitch_new * -1 * DEGtoRAD;
	//WT61P_get_yaw();
...
void state_commands(msg_pt msg){
  ...
  switch(cmd){
    ...
    case CHANGE_CONTROLLER_STATUS:
      globalControllerStatus = !globalControllerStatus;
      sprintf(Debug,"controller status: %d", globalControllerStatus);
      cli.publish(TOPIC_DEBUG, Debug);
      break;
